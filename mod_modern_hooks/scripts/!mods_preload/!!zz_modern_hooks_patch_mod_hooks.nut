if (!("mods_hookExactClass" in this.getroottable()))
{
	::mods_registerMod <- function( codeName, version, friendlyName = null, extra = null )
	{
		...
		::Hooks.errorAndQuit(format("Мод %s%s требует оригинальный 'mod_hooks (Modding Script Hooks)', который отличается от 'Modern Hooks'. Эти инструменты не конфликтуют и не заменяют друг друга, они служат для разных целей.", codeName, friendlyName));
	}
	return;
}
