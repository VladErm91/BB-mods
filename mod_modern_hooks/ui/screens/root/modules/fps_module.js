...
ModernHooksConnection.prototype.createDIV = function (_parentDiv)
{
	...
	this.mTitle = $('<div class="title title-font-very-big font-bold font-bottom-shadow font-color-title">Ошибка мода</div>');
	...
	{
		$(this).findButtonText().html("Выйти из игры");
...
ModernHooksConnection.prototype.forceQuit = function (_quit)
{
	if (_quit)
	{
		this.mTitle.text("Критическая ошибка");
		this.mFooterContainer.find(".ok-button:first").trigger('force-quit')
	}
	else
	{
		this.mTitle.text("Ошибка мода");
...
setTimeout(function(){
	if (Hooks.screenInit)
		return;
	Hooks.errorMsg = $('<div id="hooksErrorMessage">Если вы видите этот экран и игра не загружается, это, вероятно, означает, что при загрузке ваших модов что-то пошло не так.<br><br> Для получения более подробной информации проверьте лог игры, который находится по адресу: "Документы/Battle Brothers/log.html".<br><br> Если вы планируете обратиться за помощью, то обязательно предоставьте ВЕСЬ файл log.html тем, к кому вы обращаетесь за помощью.</div>');
	Hooks.errorMsg.css({
		'position' : 'absolute',
		'top' : '50%',
		'left' : '50%',
