this.lunge_goedendag <- this.inherit("scripts/skills/skill", {
	m = {
		SoundOnAttack = [
	...
	function create()
	{
		this.m.ID = "actives.lunge_goedendag";
		this.m.Name = "Прыжок вперёд";
		this.m.Description = "Быстрый выпад прямиком в цель на расстоянии 2-х клеток, сопровождаемый точным пронзающим уколом, чтобы застать цель врасплох. Чем быстрее персонаж, тем больше урона он нанесёт.";
	...
	function getTooltip()
	{
		...
		ret.extend([
			{
				...
				text = "Позволяет перемещаться мимо противников, игнорируя зону контроля"
			},
			{
				id = 6,
				type = "text",
				icon = "ui/icons/special.png",
				text = "Наносит дополнительный урон в зависимости от текущей инициативы"
			}
		]);
		...
		if (this.getContainer().getActor().getCurrentProperties().IsRooted)
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Нельзя использовать, пока обездвижен[/color]"
			});
		}
