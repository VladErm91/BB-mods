this.long_impale <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
		this.m.ID = "actives.long_impale";
		this.m.Name = "Вспороть живот";
		this.m.Description = "Мощный выпад с использованием всего веса тела нападающего.";
		this.m.KilledString = "Пронзён";
		this.m.Icon = "skills/disembovel.png";
		this.m.IconDisabled = "skills/disembovel_sw.png";
	...
	function isUsable()
	{
		return this.getContainer().getActor().getCurrentProperties().IsSpecializedInPolearms && this.m.Container.getActor().getCurrentProperties().IsAbleToUseWeaponSkills && !this.getContainer().getActor().getTile().hasZoneOfControlOtherThan(this.getContainer().getActor().getAlliedFactions());
	}
