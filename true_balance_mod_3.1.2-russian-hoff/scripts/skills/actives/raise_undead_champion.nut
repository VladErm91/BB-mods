this.raise_undead_champion <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
		this.m.ID = "actives.raise_undead_champion";
		this.m.Name = "Поднять мертвеца";
		this.m.Description = "";
	...
	function onUse( _user, _targetTile )
	{
		if (_targetTile.IsVisibleForPlayer)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " использовал умение 'Поднять мертвеца'");
		...
		_targetTile.getEntity().m.Skills.add(this.new("scripts/skills/effects/regeneration_undead_effect"));
		return true;
	}
