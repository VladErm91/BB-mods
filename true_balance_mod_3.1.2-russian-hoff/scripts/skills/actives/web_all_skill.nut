this.web_all_skill <- this.inherit("scripts/skills/skill", {
	m = {
		Cooldown = 0
	},
	function create()
	{
		this.m.ID = "actives.web_all";
		this.m.Name = "Сплести паутину";
		this.m.Description = "";
	...
	function onUse( _user, _targetTile )
	{
		...
		foreach( tile in targets )
		{
			if (tile != null)
			{
				...
				if (!targetEntity.getCurrentProperties().IsImmuneToRoot && targetEntity.getType() != this.Const.EntityType.Spider)
				{
