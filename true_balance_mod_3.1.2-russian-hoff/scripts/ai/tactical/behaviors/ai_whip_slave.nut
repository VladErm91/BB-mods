this.ai_whip_slave <- this.inherit("scripts/ai/tactical/behavior", {
	m = {
		TargetTile = null,
		PossibleSkills = [
			"actives.mod_whip_slave"
		],
		Skill = null,
		IsTravelling = false
	},
	...
	function onEvaluate( _entity )
	{
		...
		{
			if (!a.getFlags().has("slave_minion"))
			{
				continue;
			}
			...
			local isEngaged = allyTile.hasZoneOfOccupationOtherThan(a.getAlliedFactions());
	...
	function onExecute( _entity )
	{
		if (this.m.IsFirstExecuted)
		{
			if (this.m.IsTravelling)
			{
				...
				{
					this.logInfo("* " + _entity.getName() + ": Передвигается ближе, чтобы использовать умение 'Подстёгивание'");
				}
		...
		else
		{
			if (this.Const.AI.VerboseMode)
			{
				this.logInfo("* " + _entity.getName() + ": использовал умение 'Подстёгивание'!");
			}
