this.mod_ghost_agent <- this.inherit("scripts/ai/tactical/agent", {
	m = {},
	function create()
	{
		this.agent.create();
	...
	function onUpdate()
	{
		...
		if (!this.Tactical.State.isAutoRetreat() && !strategy.getStats().IsEngaged && this.m.KnownAllies.len() >= 6 && ghosts < this.m.KnownAllies.len() - 1 && this.Time.getRound() <= 1)
		{
			this.m.Properties.BehaviorMult[this.Const.AI.Behavior.ID.EngageMelee] = 0.0;
		}
		else if (!this.Tactical.State.isAutoRetreat() && !strategy.getStats().IsEngaged && this.m.KnownAllies.len() >= 6 && ghosts < this.m.KnownAllies.len() - 1 && this.Time.getRound() == 2)
		{
			this.m.Properties.BehaviorMult[this.Const.AI.Behavior.ID.EngageMelee] = 1.0;
			this.m.Properties.EngageTileLimit = 1;
			this.m.Properties.PreferWait = true;
		}
		else if (!this.Tactical.State.isAutoRetreat() && !strategy.getStats().IsEngaged && strategy.getStats().ShortestDistanceToEnemy >= 6)
		{
			this.m.Properties.BehaviorMult[this.Const.AI.Behavior.ID.EngageMelee] = 1.0;
			this.m.Properties.EngageTileLimit = 2;
			this.m.Properties.PreferWait = true;
		}
		else if (!this.Tactical.State.isAutoRetreat() && !strategy.getStats().IsEngaged && strategy.getStats().ShortestDistanceToEnemy <= 5)
		{
			this.m.Properties.BehaviorMult[this.Const.AI.Behavior.ID.EngageMelee] = 1.0;
			this.m.Properties.EngageTileLimit = 1;
		...
		foreach( t in opponents )
		{
			...
			if (t.Actor.getTile().getDistanceTo(myTile) <= 1)
			{
				opponentNearby = true;
				break;
			}
