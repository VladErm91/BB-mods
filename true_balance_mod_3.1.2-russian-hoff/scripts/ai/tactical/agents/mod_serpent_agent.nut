this.mod_serpent_agent <- this.inherit("scripts/ai/tactical/agent", {
	m = {},
	function create()
	{
		this.agent.create();
	...
	function onUpdate()
	{
		...
		if (!this.getStrategy().getStats().IsEngaged && relevantAlliesStillToAct)
		{
			this.m.Properties.EngageRangeMin = 1;
			this.m.Properties.EngageRangeMax = 3;
			this.m.Properties.EngageRangeIdeal = 3;
		}
		else if (!this.getStrategy().getStats().IsEngaged && !relevantAlliesStillToAct)
		{
			this.m.Properties.EngageRangeMin = 1;
			this.m.Properties.EngageRangeMax = 1;
			this.m.Properties.EngageRangeIdeal = 1;
		}
		else if (this.getStrategy().getStats().IsEngaged)
		{
			this.m.Properties.EngageRangeMin = 1;
			this.m.Properties.EngageRangeMax = 1;
			this.m.Properties.EngageRangeIdeal = 1;		
