this.mod_goblin_ranged_agent <- this.inherit("scripts/ai/tactical/agent", {
	m = {},
	function create()
	{
		this.agent.create();
		...
		this.m.Properties.BehaviorMult[this.Const.AI.Behavior.ID.KeepSafe] = 1.8;
		...
		this.m.Properties.TargetPriorityArmorMult = 0.1;
		this.m.Properties.OverallDefensivenessMult = 2.0;
		this.m.Properties.OverallFormationMult = 1.0;
		this.m.Properties.OverallHideMult = 0.75;
		this.m.Properties.EngageOnGoodTerrainBonusMult = 1.1;
		this.m.Properties.EngageOnBadTerrainPenaltyMult = 0.9;
	...
	function onUpdate()
	{
		...
		if (!this.Tactical.State.isAutoRetreat() && this.Time.getRound() <= 1)
		{
		...
		if (!this.Tactical.State.isAutoRetreat() && !strategy.getStats().IsEngaged && this.m.Actor.getAttackedCount() <= 3 && this.m.KnownAllies.len() >= 3 && this.Time.getRound() <= 1)
		{
